...
if(Feeds.length==0) {
	var xpathResult=document.evaluate('//a[contains(@href,".rss") or contains(@href,"=rss") or contains(@href,".atom") or contains(@href,"=atom") or ((contains(@href,"feed") or contains(@href,"rss")) and (contains(@href,"rdf") or contains(@href,"xml"))) or contains(@href,"feeds.feedburner.com")]',document,null,0,null);
	while(item=xpathResult.iterateNext()) {
		Feeds.push(item);
		control+=", "+item.href;
		}
...
function verifyFeed(n) {
	if(n<FeedLinks.length) {
		GM_xmlhttpRequest({
			...
			5D+" "+version+" "+Txt[2][lang]+" <i><b>"+scriptName+"</b></i>";
				GMSUCtextAlerte+=". "+Txt[3][lang]+" (<a href='http://userscripts.org/scripts/show/"+scriptId+"'>"+offRel+"</a>)";
				GMSUCtextAlerte+="";
				GMSUCtextAlerte+="<table><tr><td><a href='http://userscripts.org/scripts/show/"+scriptId+"'>v."+offRel+"</a></td><td><a href='http://userscripts.org/scripts/review/"+scriptId+"'>"+Txt[5][lang]+"</a></td><td><a  href='http://userscripts.org/scripts/source/"+scriptId+".user.js'>"+Txt[4][lang]+"</a></td></tr></table>"
				if(GMSUCtime>0) GMSUCtextAlerte+="<div id='GMSUC-timer'>"+GMSUCtime+" s</div>";
...
// check direct links to rss feeds not declared as <link>	
if(Feeds.length==0) {
	var xpathResult=document.evaluate('//a[contains(@href,".rss") or contains(@href,"=rss") or contains(@href,".atom") or contains(@href,"=atom") or ((contains(@href,"feed") or contains(@href,"rss")) and (contains(@href,"rdf") or contains(@href,"xml"))) or contains(@href,"feeds.fee